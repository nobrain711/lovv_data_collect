...
- 대량 또는 반복 수집은 `requests`와 `BeautifulSoup(bs4)` 기반 HTML 파싱을 기본 방식으로 사용한다.
- MediaWiki API는 HTML 파싱 결과가 부족한 소량 보강, 연결 문서 확인, 좌표·다국어명 같은 정밀 조회가 필요할 때 선택적으로 사용한다.
- API timeout, rate limit, batch 응답 누락이 발생해도 전체 수집이 중단되지 않도록 HTML 수집 결과를 우선 정규화하고, 부족한 필드만 후속 보정 대상으로 표시한다.
