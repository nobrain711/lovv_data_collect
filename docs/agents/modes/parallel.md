...
Use Parallel Mode only when the user explicitly asks for parallel agents or when Main Codex can prove that the work is independently scoped.

Parallel Mode can save wall-clock time, but it increases coordination cost and conflict risk.
...
- Load this file only when Parallel Mode is selected.
- Load only the context packet assigned to each subagent.
- Do not pass raw logs, full specs, or unrelated docs to every subagent.
- Return distilled summaries to Main Codex instead of raw intermediate output.
